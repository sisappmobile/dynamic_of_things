...
class Pushes {
  ...
  Future<void> execute() async {
    if (!Sqlites.supported) {
      if (kDebugMode) {
        print(
          "Pushes skipped: SQLite offline storage is not available on web.",
        );
      }
    ...
    List<Map<String, dynamic>> syncQueues = await DMLAssemblers.create()
        .select("*")
        .from("_sync_queues")
        .desc("created_at")
        .all();
    ...
    if (syncQueues.isNotEmpty) {
      Response response =
          await DotApis.getInstance().synchronizationPush(syncQueues);
      ...
      if (response.statusCode == 204) {
        ...
        await database.transaction((txn) async {
          for (Map<String, dynamic> syncQueue in syncQueues) {
            await txn.execute(
              "DELETE FROM _sync_queues WHERE id = ?",
              [syncQueue["id"]],
            );
          }
        });
